...
export default function Deimos() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setCambionCycle(data.cambionCycle);

        // 2. Extract Mother's Bounties
        const allSyndicates = data.syndicateMissions || [];
        const cambionData = allSyndicates.find(
          (syn) => syn.syndicateKey === 'Entrati'
        );

        // Ensure jobs exist before setting them
        if (cambionData && cambionData.jobs) {
          setBounties(cambionData.jobs);
        } else {
          setBounties([]);
        }
      }
  ...
  if (loading) {
    return <div className="p-8 text-slate-400">Loading Deimos data...</div>;
  }

  const isFass = cambionCycle?.state === 'Fass';
    ...
    <div className="w-full max-w-7xl space-y-6">
      ...
      <div className={`p-6 rounded-xl border relative overflow-hidden transition-colors duration-500 ${
        ...
        <div className={`absolute -top-24 -right-24 w-64 h-64 rounded-full blur-3xl opacity-20 pointer-events-none ${
          isFass ? 'bg-amber-400' : 'bg-blue-500'
        }`}></div>

        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white mb-1">Deimos</h1>
            <p className="text-slate-400">Cambion Drift</p>
          </div>

          <div className="text-right">
            <div className={`text-2xl font-bold uppercase tracking-wider mb-1 ${
              isFass ? 'text-amber-400' : 'text-blue-400'
            }`}>
              {isFass ? 'Fass' : 'Vome'}
            </div>
            <div className="text-slate-300">
              Time remaining: <Countdown expiry={cambionCycle?.expiry} />
            </div>
          </div>
        </div>
      </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-slate-200">Mother's Bounties</h2>
          <span className="text-sm text-slate-400">Entrati Syndicate</span>
        </div>

        {entrati_bounties && entrati_bounties.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {entrati_bounties.map((job, index) => {
